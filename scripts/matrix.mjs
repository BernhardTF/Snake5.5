...
const combos = [
  ['arcade','grid','erg'], ['zen','glide','lagoon'], ['timeattack','grid','svartsandur'], ['classic','glide','salar'], ['daily','grid','karesansui'],
];
for (const [mode, movement, biome] of combos) {
  await page.evaluate((r) => window.__app.startGame(r), { mode, movement, biome, skin: 'obsidian' });
  ...
  await page.screenshot({ path: `.qa/matrix-${mode}-${movement}-${biome}.png` });
  console.log(mode, movement, biome, JSON.stringify(s), errs.length ? 'ERRORS: ' + errs.splice(0).join(' | ') : 'ok');
